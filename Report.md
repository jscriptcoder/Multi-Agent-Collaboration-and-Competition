...
Another aspect worth to investigate would be to not perform early stop, that is, not to stop the learning after reaching the target score 100 times consecutively, but instead leave the training go for a fixed amount of episodes, let's say 10 or 20k, and see how the the policies evolve. Probably there we could see that TD3 and SAC evolve more efficiently than DDPG eventually coming up with better policies.
